...
Game.onload = function () {
    ...
    Game.gameObjects.paddle = new DE.GameObject({
        ...
        clamp: function (x) {
            return Math.min(Math.max(x, Game.properties.walls.width + Game.properties.paddle.width / 2), Game.properties.screen.size.width - Game.properties.walls.width - Game.properties.paddle.width / 2);
        },
        ...
        checkCollision: function (position, velocity) {
            ...
            if (position.x >= (this.x - this.vars.size.width / 2) && position.x <= (this.x + this.vars.size.width / 2) && position.y >= (this.y - this.vars.size.height / 2 - Game.properties.balls.size / 2) && position.y <= (this.y - this.vars.size.height / 2)) {
                collision = true;
        ...
        updateSpawnAnimation: function () {
            if (!Game.gameObjects.scriptHolder.vars.spawnAnimation.completed) {
                this.y = -Game.properties.paddle.verticalOffset + Game.gameObjects.scriptHolder.vars.spawnAnimation.easedTime * Game.properties.screen.size.height;
            }
        }
    });
    Game.gameObjects.walls = new DE.GameObject({
        ...
        y: 0,
        renderers: [
            new DE.RectRenderer(Game.properties.walls.width * 2, Game.properties.screen.size.height, Game.properties.walls.color, { // Left
                x: -Game.properties.walls.width,
                fill: true
            }),
            new DE.RectRenderer(Game.properties.walls.width * 2, Game.properties.screen.size.height, Game.properties.walls.color, { // Right
                x: Game.properties.screen.size.width - Game.properties.walls.width,
                fill: true
            }),
            new DE.RectRenderer(Game.properties.screen.size.width, Game.properties.walls.width * 2, Game.properties.walls.color, { // Top
        ...
        checkCollision: function (position, velocity) {
            ...
            if (velocity.x < 0 && position.x <= Game.properties.walls.width) { // Left
                collision = true;
                newVelocity.x = Math.abs(velocity.x);
            } else if (velocity.x > 0 && position.x >= (Game.properties.screen.size.width - Game.properties.walls.width)) { // Right
                collision = true;
                newVelocity.x = -Math.abs(velocity.x);
            }

            if (velocity.y < 0 && position.y <= Game.properties.walls.width) { // Top
                collision = true;
                newVelocity.y = Math.abs(velocity.y);
            }
    ...
    Game.gameObjects.balls.push(new DE.GameObject({
        vars: {
            stuck: true,
            velocity: {x: 0, y: 0}
        },
        x: Game.properties.screen.size.width / 2,
        y: Game.properties.screen.size.height - Game.properties.paddle.verticalOffset - Game.properties.paddle.height - Game.properties.paddle.ballSpacing,
        ...
        automatisms: [["updatePosition"], /*["updateRotation"]*/],
        ...
        setLaunchVelocity: function () {
            this.vars.velocity = {x: (Math.random() > 0.5 ? -1 : 1) * (Math.floor(Math.random() * (15 - 5) ) + 5), y: -20};
        }
    }));
            ...
            Game.gameObjects.bricks.push(new DE.GameObject({
                ...
                checkCollision: function(position, velocity) {
                    if (this.vars.active) {
                        ...
                        if (velocity.y < 0 && (this.x - Game.properties.bricks.width / 2) <= position.x && position.x <= (this.x + Game.properties.bricks.width / 2) && (this.y + Game.properties.bricks.height / 2) <= position.y && position.y <= (this.y + Game.properties.bricks.height / 2 + Game.properties.balls.size / 2)) { // Bottom
                            collision = true;
                            newVelocity.y = Math.abs(velocity.y);
                        } else if (0 < velocity.y && (this.x - Game.properties.bricks.width / 2) <= position.x && position.x <= (this.x + Game.properties.bricks.width / 2) && (this.y - Game.properties.bricks.height / 2 - Game.properties.balls.size / 2) <= position.y && position.y <= (this.y - Game.properties.bricks.height / 2)) { // Top
                            collision = true;
                            newVelocity.y = -Math.abs(velocity.y);
                        } else if (velocity.x < 0 && (this.y - Game.properties.bricks.height / 2) <= position.y && position.y <= (this.y + Game.properties.bricks.height / 2) && (this.x + Game.properties.bricks.width / 2) <= position.x && position.x <= (this.x + Game.properties.bricks.width / 2 + Game.properties.balls.size / 2)) { // Right
                            collision = true;
                            newVelocity.x = Math.abs(velocity.x);
                        } else if (0 < velocity.x && (this.y - Game.properties.bricks.height / 2) <= position.y && position.y <= (this.y + Game.properties.bricks.height / 2) && (this.x - Game.properties.bricks.width / 2 - Game.properties.balls.size / 2) <= position.x && position.x <= (this.x - Game.properties.bricks.width / 2)) { // Left
                            collision = true;
                            newVelocity.x = -Math.abs(velocity.x);
                        }
    ...
    Game.scene.add(
        Game.gameObjects.walls,
        Game.gameObjects.paddle,
        Game.gameObjects.bricks,
        Game.gameObjects.balls,
        Game.gameObjects.scriptHolder
    );
}
